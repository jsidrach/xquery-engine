...
public class XQueryVisitor extends edu.ucsd.cse232b.jsidrach.antlr.XQueryBaseVisitor<LinkedList<Node>> {
     ...
     * XQuery (join)
     * <pre>
     * [join(xq_1, xq_2, [tag_1, ..., tag_n], [tag_n+1, ..., tag_2n])](C)
     *   → { x ∪ y | x ← [xq_1](C), y ← [xq_2](C) if x/tag_1/* = y/tag_n+1/* ∧ ... ∧ x/tag_n/* = y/tag_2n/* }
     * </pre>
     *
     * @param ctx Current parse tree context
    ...
    public LinkedList<Node> visitXqJoin(XQueryParser.XqJoinContext ctx) {
        ...
        if (right.size() < left.size()) {
            LinkedList<Node> swap = left;
            List<TerminalNode> swapTags = leftTags;
            left = right;
            leftTags = rightTags;
            right = swap;
            rightTags = swapTags;
        }
        // Add all left nodes to the hash
        ...
        for (Node l : left) {
            ...
            if (eqVars.containsKey(key)) {
                ...
                eqVars.put(key, XQueryEvaluator.singleton(l.cloneNode(true)));
            }
        }
        ...
        for (Node r : right) {
            ...
            if (eqVars.containsKey(key)) {
                ...
                LinkedList<Node> ls = eqVars.get(key);
                for (Node l : ls) {
                    for (Node rChild : rChildren) {
                        l.appendChild(rChild.cloneNode(true));
                    }
                }
                nodes.addAll(ls);
            }
     ...
     * <pre>
     * [for Var_1 := xq_1, ..., Var_n := xq_n
     *  let Var_n+1 := xq_n+1, ..., Var_n+k = xq_n+k
     *  where Cond
     *  return xq_n+k+1](C)
     *   → { [xq_n+k+1](C_n+k) | v_1 ← [xq_1](C_0), ..., v_n ← [xq_n](C_n-1) if [Cond](C_n+k) }
     *   where C_0 := C, C_i := { Var_i → [xq_i](C_i-1) } ∪ C_i-1, i ∈ [1, ..., n]
     *     and C_j := { Var_j → [xq_j](C_j-1) } ∪ C_j-1, j ∈ [n+1, ..., n+k]
     * </pre>
     *
     * @param ctx Current parse tree context
     ...
     */
    @Override
    public LinkedList<Node> visitTagList(XQueryParser.TagListContext ctx) {
